...
from sizechart_detect.detect.ctpn_model import CTPN_Model
from sizechart_detect.detect.ctpn_utils import gen_anchor, bbox_transfor_inv, clip_box, filter_bbox,nms, TextProposalConnectorOriented
from sizechart_detect.detect.ctpn_utils import resize
...
def get_det_boxes(image, model, display = False, expand = True):
    ...
    with torch.no_grad():
        ...
        if display:
            ...
            for i, box in enumerate(select_anchor):
                pt1 = (box[0], box[1])
                pt2 = (box[2], box[3])
                blank = cv2.rectangle(blank, pt1, pt2, (50, 0, 0), -1)
                # cv2.imwrite("../test_result/"+str(i)+".jpg",blank)
...
if __name__ == '__main__':
    img_path = '../../data/image_data/6.jpg'
    image = cv2.imread(img_path)
    text,image,image_r = get_det_boxes(image, model)
    dis(image)